...
#define logging_filename(x) strrchr(x, '/') ? strrchr(x, '/') + 1 : x
...
#define LOG_FATAL(format, ...)                                                                                    \
  do {                                                                                                            \
    ...
    if (level >= LOG_LEVEL_FATAL)                                                                                 \
      DEBUG("\033[;31m[FATAL] %s %s:%d: " format "\n\033[0m", TIME, logging_filename(__FILE__), __LINE__, ##__VA_ARGS__); \
    exit(1);                                                                                                      \
  } while (0)
...
#define LOG_ASSERT(condition, format, ...)                                                                 \
  if (!(condition)) {                                                                                      \
    GET_TIME                                                                                               \
    DEBUG("\033[;31m %s Assertion Failed! %s:%d: " format "\n\033[0m", TIME, logging_filename(__FILE__), __LINE__, \
          ##__VA_ARGS__);                                                                                  \
    exit(1);                                                                                               \
  }
#else
